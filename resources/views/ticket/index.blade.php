        ...
        <table class="w-full overflow-x-auto">
            <tbody class="mt-5">
                @forelse ($tickets as $each)
                            ...
                            <td colspan="14" class="text-black font-inter font-semibold py-3">
                                <div class="flex justify-between">
                                    <div class="status_count flex items-center">
                                        <span>{{ ucfirst($each?->name) . ' Request' }} {{ '(' . $each?->ticket_count . ')' }}</span>
                                    ...
                                    <div class="status">
                                        <a href="{{ route('admin.ticket.viewAll', ['ticket_status_id' => $each?->id]) }}" class="border border-slate-300 rounded font-inter font-normal px-2 py-1">View All</a>
                                    </div>
                                </div>
                            </td>
                        </tr>
                    ...
                    <tr class="rounded shadow submenu" style="display: table-row;">
                        <td colspan="15">
                            <table class="w-full overflow-x-auto">
                                ...
                                <tbody>
                                    @foreach ($each?->ticket as $ticket)
                                        <tr>
                                            <td class="p-2" style="max-width: 70px">
                                                <span class="font-inter font-normal text-slate-500 text-sm">
                                                    #{{ $ticket?->id }}
                                                </span>
                                            </td>
                                            <td class="p-2 font-normal text-gray-400" style="width: 200px">
                                                <span class="font-inter font-normal text-slate-500 text-sm">
                                                    {{ $ticket?->title }}
                                                </span>
                                            </td>
                                            <td class="p-2 font-normal text-gray-400" style="max-width: 80px">
                                                <span class="text-{{ $ticket?->priority == 'high' ? 'high' : 'medium' }}-400 font-inter font-bold">
                                                    {{ $ticket?->priority }}
                                                </span>
                                            </td>
                                            <td class="p-2 font-normal text-gray-400" style="max-width: 130px">
                                                @if (strtolower(trim($ticket?->ticket_status?->name)) === 'in process')
